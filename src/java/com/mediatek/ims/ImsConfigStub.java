...
public class ImsConfigStub extends IImsConfig.Stub {
    ...
    public void setFeatureValue(int feature, int network, int value, ImsConfigListener listener) {
        ...
        switch (feature) {
            case ImsConfig.FeatureConstants.FEATURE_TYPE_VOICE_OVER_LTE:
            case ImsConfig.FeatureConstants.FEATURE_TYPE_VIDEO_OVER_LTE:
                int oldValue = Settings.Global.getInt(
                        mContext.getContentResolver(),
                        Settings.Global.IMS_SWITCH, 0);
                if (value != oldValue) {
    ...
    public void getVideoQuality(ImsConfigListener listener) {
        Log.w(TAG, "getVideoQuality(" + listener + "): stub!");
        try {
            listener.onGetVideoQuality(
                    ImsConfig.OperationStatusConstants.UNSUPPORTED_CAUSE_NONE,
    ...
    public void setVideoQuality(int quality, ImsConfigListener listener) {
        Log.w(TAG, "setVideoQuality(" + quality + ", " + listener + "): stub!");
        try {
            listener.onSetVideoQuality(ImsConfig.OperationStatusConstants.UNSUPPORTED_CAUSE_NONE);
        } catch (RemoteException e) {
            Log.e(TAG, "remote exception happened", e);
        }
    }
    ...
    public void getPacketCount(ImsConfigListener listener) {
        Log.w(TAG, "getPacketCount(" + listener + "): stub!");
        try {
            listener.onGetPacketCount(ImsConfig.OperationStatusConstants.UNSUPPORTED_CAUSE_NONE, 0L);
        } catch (RemoteException e) {
            Log.e(TAG, "remote exception happened", e);
        }
    }
    ...
    public void getPacketErrorCount(ImsConfigListener listener) {
        Log.w(TAG, "getPacketErrorCount(" + listener + "): stub!");
        try {
            listener.onGetPacketErrorCount(ImsConfig.OperationStatusConstants.UNSUPPORTED_CAUSE_NONE, 0L);
        } catch (RemoteException e) {
            Log.e(TAG, "remote exception happened", e);
        }
    }
    ...
    @Override
    public void getWifiCallingPreference(ImsConfigListener listener) {
        Log.w(TAG, "getWifiCallingPreference(" + listener + "): stub!");
        try {
            listener.onGetWifiCallingPreference(
                    ImsConfig.OperationStatusConstants.UNSUPPORTED_CAUSE_NONE,
                    ImsConfig.WifiCallingValueConstants.NOT_SUPPORTED,
    ...
    public void setWifiCallingPreference(int wifiCallingStatus, int wifiCallingPreference,
        ...
        Log.w(TAG, "setWifiCallingPreference(" + wifiCallingStatus + ", " + wifiCallingPreference + ", " + listener + "): stub!");
        try {
            listener.onSetWifiCallingPreference(ImsConfig.OperationStatusConstants.UNSUPPORTED_CAUSE_NONE);
        } catch (RemoteException e) {
            Log.e(TAG, "remote exception happened", e);
        }
    }
