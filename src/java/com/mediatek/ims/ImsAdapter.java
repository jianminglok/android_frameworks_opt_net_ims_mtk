...
public class ImsAdapter extends BroadcastReceiver {
    ...
    public class VaSocketIO extends Thread {
        ...
        public VaSocketIO(String socket_name) {
            ...
            buf = new byte[8];
            Rlog.d(TAG, "VaSocketIO(): Enter");
        }

        public void run() {
            Rlog.d(TAG, "VaSocketIO(): Run");
            while (true) {
                if (misImsAdapterEnabled) {
                    ...
                    try {
                        ...
                        e.printStackTrace();
                        Rlog.d(TAG, "VaSocketIO(): InterruptedIOException (" + (doTrm == true ? "1" : "0" ) + ")");
                    } catch (Exception e) {
                        disconnectSocket();
                        e.printStackTrace();
                        if (misImsAdapterEnabled && (IS_USER_BUILD || IS_USERDEBUG_BUILD)) {
                        ...
                        Rlog.d(TAG, "VaSocketIO(): Exception (" + (doTrm == true ? "1" : "0" ) + ")");
                    }
                    
                    if (doTrm == true) {
                        int trmPhoneId = Util.getDefaultVoltePhoneId();
                        Rlog.d(TAG, "VaSocketIO(): recover Phone (trmPhoneId="
                                + trmPhoneId + ")");
                        try {
                            getITelephonyEx().setTrmForPhone(trmPhoneId, 2);
                        } catch (RemoteException re) {
                            Rlog.d(TAG, "VaSocketIO: phone trm exception (re: "+re.getMessage()+")");
                        } catch (NullPointerException npex) {
                            // This could happen before phone restarts due to crashing
                            Rlog.d(TAG, "VaSocketIO: phone trm exception (npex: "+npex.getMessage()+")");
                        }
                    ...
                    synchronized (VaSocketIOThreadLock) {
                        try {
                            Rlog.d(TAG, "VaSocketIO(): thread \""+Thread.currentThread().getId()+"\" enter wait state");
                            VaSocketIOThreadLock.wait();
                            Rlog.d(TAG, "VaSocketIO(): thread \""+Thread.currentThread().getId()+"\" leave wait state");
                        } catch (InterruptedException ie) {
                            Rlog.d(TAG, "VaSocketIO(): waiting thread \""+Thread.currentThread().getId()+"\" interrupted ("+ie.getMessage()+")");
                        }
                    }
                }
            }
        ...
        public boolean connectSocket() {
            Rlog.d(TAG, "connectSocket() Enter");
            ...
            try {
                ...
                Rlog.d(TAG, "connectSocket() update socket phone Id: " + mPhoneId);
        ...
        public void disconnectSocket() {
            Rlog.d(TAG, "disconnectSocket() Enter, mOut=" + mOut);
            try {
                if (mSocket != null) {
                    mSocket.close();
                }
                ...
                Rlog.d(TAG, "disconnectSocket() reset socket phone Id");
            }
        }
            ...
            try {
                synchronized (this) {
                    if (mOut != null) {
                        ...
                        Rlog.d(TAG, "mOut is null, socket is not setup");
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
        ...
        private void dumpEvent(VaEvent event) {
            Rlog.d(TAG, "dumpEvent: phone_id:" + event.getPhoneId()
    ...
    public ImsAdapter(Context context) {
        ...
        Rlog.d(TAG, "ImsAdapter(): ImsAdapter Enter");
    ...
    public void enableImsAdapter() {
        Rlog.d(TAG, "enableImsAdapter: misImsAdapterEnabled="
        ...
        public void run() {
            ...
            if (misImsAdapterInit) {
                if (mIO.connectSocket() == true) {
                    Rlog.d(TAG, "EnableImsRunnable(): connectSocket success");
                    ...
                    Rlog.d(TAG, "EnableImsRunnable(): connectSocket error");
    ...
    private void enableImsAdapter2ndStage() {
        Rlog.d(TAG, "enableImsAdapter2ndStage()Enter");
        ...
        Rlog.d(TAG, "enableImsAdapter2ndStage(): Va process started!");
    ...
    public void disableImsAdapter(boolean isNormalDisable) {
        ...
        Rlog.d(TAG, "disableImsAdapter(): misImsAdapterEnabled="
                + misImsAdapterEnabled + ", misImsAdapterInit="
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        Rlog.d(TAG, "onReceive, intent action is " + action);
    ...
    public void ImsServiceUp() {
        mImsServiceUp = true;
        Rlog.d(TAG, "ImsServiceUp, start to ACTION_IMS_SERVICE_UP intent");
/*
