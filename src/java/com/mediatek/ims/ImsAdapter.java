...
public class ImsAdapter extends BroadcastReceiver {
    ...
    public class VaSocketIO extends Thread {
        ...
        public VaSocketIO(String socket_name) {
            ...
            buf = new byte[8];
            Log.d(TAG, "VaSocketIO(): Enter");
        }

        public void run() {
            Log.d(TAG, "VaSocketIO(): Run");
            while (true) {
                if (misImsAdapterEnabled) {
                    try {
                        ...
                        e.printStackTrace();
                        Log.d(TAG, "VaSocketIO(): InterruptedIOException");
                    } catch (Exception e) {
                        disconnectSocket();
                        e.printStackTrace();
                        if (misImsAdapterEnabled && (IS_USER_BUILD || IS_USERDEBUG_BUILD)) {
                        ...
                        Log.d(TAG, "VaSocketIO(): Exception");
                    }
                }
            }
        }
        ...
        public void disconnectSocket() {
            Log.d(TAG, "disconnectSocket() Enter, mOut=" + mOut);
            try {
                if (mSocket != null) {
                    mSocket.close();
                }
            ...
            try {
                synchronized (this) {
                    if (mOut != null) {
                        ...
                        Log.d(TAG, "mOut is null, socket is not setup");
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
        ...
        private void dumpEvent(VaEvent event) {
            Log.d(TAG, "dumpEvent: reqiest_id:" + event.getRequestID() + "data_len:" + event.getDataLen() + ",event:" + event.getData());
        }
    ...
    public ImsAdapter(Context context) {
        ...
        Log.d(TAG, "ImsAdapter(): ImsAdapter Enter");
    ...
    public void enableImsAdapter() {
        Log.d(TAG, "enableImsAdapter: misImsAdapterEnabled=" + misImsAdapterEnabled);
    ...
    private class EnableImsRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (mIO.connectSocket() == true) {
                Log.d(TAG, "EnableImsRunnable(): connectSocket success");
                ...
                Log.d(TAG, "EnableImsRunnable(): connectSocket error");
    ...
    public void disableImsAdapter(boolean isNormalDisable) {
        ...
        Log.d(TAG, "disableImsAdapter(): misImsAdapterEnabled=" + misImsAdapterEnabled + ", isNormalDisable=" + isNormalDisable);
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        Log.d(TAG, "onReceive, intent action is " + action);
    ...
    public void ImsServiceUp() {
        mImsServiceUp = true;
        Log.d(TAG, "ImsServiceUp, start to ACTION_IMS_SERVICE_UP intent");
/*
